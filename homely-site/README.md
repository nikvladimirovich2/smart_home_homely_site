...
- Форма на `contacts.html` отправляет данные через `mailto:` — при
  сабмите открывается почтовый клиент посетителя с предзаполненным
  письмом на `CONFIG.contact.email` (см. `buildMailtoLink()` в
  `assets/js/main.js`). Это работает без бэкенда, но требует, чтобы у
  посетителя был настроен почтовый клиент — часть пользователей (особенно
  на мобильных с почтой только в браузере) не завершат отправку. Для
  более надёжной доставки подключите сервис вроде Formspree, собственный
  backend или serverless-функцию в `initContactForm()`.
